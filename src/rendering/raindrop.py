...
class Raindrop:
    def __init__(self, screen_width, wind_speed, image):
        ...
        self.y_vel = random.uniform(15, 20) 
    ...
    def fall(self, raindrop_effect_particles):
        self.x += self.x_vel
        self.y += self.y_vel
    ...
    def update(self, raindrop_effect_particles, surface):
        self.fall(raindrop_effect_particles)
        self.draw(surface)
...
class Particle:
    ...
    def draw(self, surface):
        pygame.draw.circle(surface, (102, 212, 249), (int(self.x), int(self.y)), self.radius)